...
export const DomTreeItem = ({ depth, expanded, found, item, marks, selectedPath, onSelect, onToggle }: DomTreeItemProps) => {
    ...
    return (
        <li className="tree-node">
            <div className={`tree-row${selected ? ' is-selected' : ''}${diff ? ' is-diff' : ''}${match ? ' is-found' : ''}`} style={{ marginLeft: `${depth * 18}px` }}>
            ...
            {hasChildren && open ? <ul className="tree-list">{item.items.map((child) => <DomTreeItem key={child.path} depth={depth + 1} expanded={expanded} found={found} item={child} marks={marks} selectedPath={selectedPath} onSelect={onSelect} onToggle={onToggle} />)}</ul> : null}
        </li>
    );
};
